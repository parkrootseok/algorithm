...
/**
 * Binary Search Upper Bound(이진 탐색 - 상한)
 * - O(log n)
 * - 정렬 필수
 * - 탐색 값을 초과하는 첫 위치를 탐색
 * @author parkrootseok
 ...
 * 2. 중간 위치를 초기화
 * 3. 탐색값 < 중앙값 이라면, 중앙값보다 왼쪽에 위치하므로 오른쪽 범위를 축소(단, 이상인 값 중 첫 값이므로 중간 지점으로 초기화)
...
public class binarySearchUpperBound {
	...
	public static int binarySearchUpperBound(int target) {
