    			...
    			if (start != people) {
    				...
    				for (int destination = 0; destination < peopleNumber; destination++) {
        				...
        				if (start != destination && destination != people) {
        					...
        					// start에서 destination으로 바로 갈 때의 비용과 people을 경유하여 destination으로 갈 때의 비용을 비교하여 최소값인 경우로 갱신
            				influences[start][destination] 
            						= Math.min(influences[start][destination], influences[start][people] + influences[people][destination]);
